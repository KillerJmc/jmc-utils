...
public class StrsTest {
    ...
    @Test
    public void orSplitTest() {
        var cmd = "cd / && ls -al; rm -rf *.log; cat machines.txt || grep name; echo succeed > result.txt";
        Strs.orSplit(cmd, ";", "&&", "||")
                .stream()
                .map(String::trim)
                .forEach(System.out::println);
        System.out.println(Strs.orSplit("11uu22ttt33uuu44vvv55", "tt", "uu", "vvv"));
    }
    ...
    public void subTest() {
        var s = "abcdefg";
        Assert.assertEquals(Strs.subExclusive(s, "a", "fg"), "bcde");
        Assert.assertEquals(Strs.subExclusive(s, "a"), "bcdefg");
        Assert.assertEquals(Strs.subInclusive(s, "cd", "f"), "cdef");
        Assert.assertEquals(Strs.subInclusive(s, "cd"), "cdefg");
    }
